...
r <- 35  ### remaining dimension after data reduction; namely, reducing from n by p to n by r, where r << p.

M <- list()
for (i in 1: grp) {
  M[[i]] =  t(fread(paste0("M", i, ".csv")))   ### Read data M[[i]], each is a n_i by p matrix containing data 
...
for (i in 1:(m+1)) {
  ...
  VV[[i]] = tcrossprod(t(fread(paste0("V_tilde_", i, ".csv"))))
...
for (i in 1:(m+1)) {
  ...
  A_tilde[[i]] = as.matrix(fread(paste0("V_tilde_", i, ".csv")))  %*% CB_inv
...
for (i in 1:grp) {
  VD_r[[i]] =  tcrossprod( (as.matrix(t(fread(paste0("V", i, ".csv"))))[1:r,]), D_r)   ## here the V_i is from M_k = U_k Sigma_k V_k
}
...
for (i in 1:grp) {
  V_rk_r[[i]] =  diag(fread(paste0("Sigma", i, ".csv"))$x ) [, 1:r] %*% VD_r[[i]]     ### choose the first n_pca number of sigma
