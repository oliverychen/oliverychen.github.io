...
<br>
Will give you satisfaction, you may have ’t.</p>

- <i>Othello:</i> Act 3 Scene 3
<br>
...
<div id="top">
    <a id="logo" href="<?php echo SITE_URL?>" target="_blank">
        <img src="{{ site.baseurl }}/images/Arnolfini_Portrait.jpg" alt="HTML5 Icon" style="height:250px;">
    </a>
    <img src="{{ site.baseurl }}/images/The_Arnolfini_Portrait_2.jpg" alt="HTML5 Icon" style="height:250px;">
    <figcaption>Left: What we observe; 
        Right: What you may have missed, but not hidden</figcaption>
    <figcaption><i>The Arnolfini Portrait (1434)</i> by Jan van Eyck. Oil on oak. </figcaption>
</div>
</center>
...
<div id="top">
    <a id="logo" href="<?php echo SITE_URL?>" target="_blank">
        <img src="{{ site.baseurl }}/images/Hidden.jpg" alt="HTML5 Icon" style="height:250px;">
    </a>
    <img src="{{ site.baseurl }}/images/mona-lisas-smile-self-portrait.jpg" alt="HTML5 Icon" style="height:250px;">
    <figcaption>Left: What you observe, and what is "hidden" underwater (yet likely exists); 
        Right: What you observe, and what is truly "hidden" (only assumptions exist)</figcaption>
    <figcaption><i>Mona Lisa</i> Image source: Wikimedia. </figcaption>
</div>
</center>
...
<br>
Fundamental to these is a process called the hidden Markov model where observations such as smiles and nucleotides are generated by a hidden state such as underlying mood and a set of exons, splice sites, and introns.
...
<br>
4. It allows us to study the parameter  <img center src="http://latex.codecogs.com/gif.latex?
\theta
" border="0"/>; but what is <img center src="http://latex.codecogs.com/gif.latex?
\theta
" border="0"/> here?
<br>
5. To estimate <img center src="http://latex.codecogs.com/gif.latex?
\theta
" border="0"/>, we need the EM (expectation maximization) algorithm;
<br> The HMM involves many interesting theories, including probability theory, graph theory, Bayesian theory, and structual equation.

<h5>2. Definition (in the context of dynamic connectivity) </h5>

Define <img center src="http://latex.codecogs.com/gif.latex?
S_1, S_2, \cdots, S_T
" border="0"/> as hidden states over T time points (a time point here could be referred to as a time window) <img center src="http://latex.codecogs.com/gif.latex?
\mathcal{C}_1, \mathcal{C}_2, \cdots, \mathcal{C}_T
" border="0"/> are dynamic connectivity matrices over T time points. For any <img center src="http://latex.codecogs.com/gif.latex?
i \in \{1, 2, \cdots, T \}
" border="0"/>, we have <img center src="http://latex.codecogs.com/gif.latex?
S_i \in \{1, 2, \cdots, k \}
" border="0"/>; namely there are k hidden states.

<br>
Referring <img center src="http://latex.codecogs.com/gif.latex?
\{S_i, \mathcal{C}_i  \}_{ 1 \leq i \leq T}
" border="0"/> to a HMM, we imply

<br> 
[A] <img center src="http://latex.codecogs.com/gif.latex?
S_i \rightarrow \mathcal{C}_i, \forall i;
" border="0"/>

<br>
[B] <img center src="http://latex.codecogs.com/gif.latex?
S_i \sim Markov(n)
" border="0"/>, where <img center src="http://latex.codecogs.com/gif.latex?
Markov(n)
" border="0"/> infers the <img center src="http://latex.codecogs.com/gif.latex?
n^{\text{th}}
" border="0"/> order Markov.

<br> 
Above [A] and [B] can be represented in two ways: graphically, and probablistically.

<br> For n = 1, graphically, [A] and [B] can be drawn as follows:
<center>
<div id="top">
    <a id="logo" href="<?php echo SITE_URL?>" target="_blank">
        <img src="{{ site.baseurl }}/images/Markov.jpg" alt="HTML5 Icon" style="width:300px;">
        
    </a>
</div>
</center>
<br> For n = 1, probabilistically, we have
<img center src="http://latex.codecogs.com/gif.latex?
\mathbb{P} \bigg(S_1, S_2, \cdots, S_T; \mathcal{C}_1, \mathcal{C}_2, \cdots, \mathcal{C}_T \bigg)
= \bigg[\prod_{t=2} ^{t=T} \mathbb{P} (S_t \vert S_{t-1}) \mathbb{P}(\mathcal{C}_t \vert S_t) \bigg]   \mathbb{P} (\mathcal{C}_1 \vert S_1) \mathbb{P} (S_1)
" border="0"/>, 
where three types of probabilities need to be specified

<br>
[B1] <img center src="http://latex.codecogs.com/gif.latex?
\mathbb{P}(S_i);
" border="0"/>

<br> 
[B2] A <i>trainsitioning matrix</i> 
<img center src="http://latex.codecogs.com/gif.latex?
T_{K \times K}
" border="0"/> where <img center src="http://latex.codecogs.com/gif.latex?
T_{ij}  = \mathcal{P}(S_l = j \vert S_{m} = i),
" border="0"/>
<img center src="http://latex.codecogs.com/gif.latex?
\forall l, m \in \{1, \cdots, T \}, \forall i, j \in \{1, \cdots, K \}.
" border="0"/>

<br> [B3] An <i>emission matrix</i> 
<img center src="http://latex.codecogs.com/gif.latex?
\mathcal{E}_{K \times L}
" border="0"/>, (assuming <img center src="http://latex.codecogs.com/gif.latex?
\mathcal{C}_t
" border="0"/> is discret and <img center src="http://latex.codecogs.com/gif.latex?
\mathcal{C}_t \{1, \cdots, L\}
" border="0"/>) where <img center src="http://latex.codecogs.com/gif.latex?
\mathcal{E}_{kl}  = \mathcal{P}(\mathcal{C}_i = l \vert S_i = k),
" border="0"/> 
<img center src="http://latex.codecogs.com/gif.latex?
\forall i \in \{1, \cdots, T \}, \forall k \in \{1, \cdots, K \}, \forall l \in \{1, \cdots, L \}.
" border="0"/> 


<h5>3. Some Graph Theory </h5>

Consider three nodes A, B, and C, where A goes into (by into there is a sense of direction) B, and B goes into C. Then we say 
<center>
A is the <i>parent</i> of B (or B is the <i>child</i> of A);
</center>
<center>
B is the <i>parent</i> of C (or C is the <i>child</i> of B);
</center> 
<center>
C is the <i>child's of child</i> of A (or A is the <i>parent's parent</i> of C). 
</center>

<br>
If A goes into B without goes through another node, then we say the path from A to B is a <i>directed path</i>. If, however, A goes into B via some another node(s), then we say the path from A to B is an <i>undirected path</i>.
...
<div id="top">
    <a id="logo" href="<?php echo SITE_URL?>" target="_blank">
        <img src="{{ site.baseurl }}/images/GT.jpg" alt="HTML5 Icon" style="width:450px;">
...
\mathcal{C}
" border="0"/> be three sets of nodes. We say 
<img center src="http://latex.codecogs.com/gif.latex?
\mathcal{A} \perp \mathcal{B} \vert \mathcal{C}
" border="0"/>
if along every undirected path between <img center src="http://latex.codecogs.com/gif.latex?
a \in \mathcal{A}
" border="0"/> and <img center src="http://latex.codecogs.com/gif.latex?
b \in \mathcal{B}
" border="0"/> there exists a node <img center src="http://latex.codecogs.com/gif.latex?
d
" border="0"/> such that 
(1) <img center src="http://latex.codecogs.com/gif.latex?
d
" border="0"/> has converging arrows (arrows coming both into <img center src="http://latex.codecogs.com/gif.latex?
d
" border="0"/>) and neither <img center src="http://latex.codecogs.com/gif.latex?
d
" border="0"/> nor its descendents are in <img center src="http://latex.codecogs.com/gif.latex?
\mathcal{C}
" border="0"/>; 
<br>
(2) <img center src="http://latex.codecogs.com/gif.latex?
d
" border="0"/> does not have converging arrows and <img center src="http://latex.codecogs.com/gif.latex?
d \in \mathcal{C}.
" border="0"/>

<h5>4. Dynamic Bayesian Network</h5>

Besides the first order and higher order Markov models we briefly discussed above. Critical to hidden Markov models are state space models. To me, this is how the model assumptions are put forward to estimation, through a transition function <img center src="http://latex.codecogs.com/gif.latex?
f_t
" border="0"/>. 

<br> 
Recall <img center src="http://latex.codecogs.com/gif.latex?
\mathbb{P} \bigg(S_1, S_2, \cdots, S_T; \mathcal{C}_1, \mathcal{C}_2, \cdots, \mathcal{C}_T \bigg)
= \bigg[\prod_{t=2} ^{t=T} \mathbb{P} (S_t \vert S_{t-1}) \mathbb{P}(\mathcal{C}_t \vert S_t) \bigg]   \mathbb{P} (\mathcal{C}_1 \vert S_1) \mathbb{P} (S_1)
" border="0"/>. 

<br>
With a state space model, we let 
<img center src="http://latex.codecogs.com/gif.latex?
S_t = f_t (S_{t-1}) + \epsilon_t
" border="0"/>
where <img center src="http://latex.codecogs.com/gif.latex?
f_t
" border="0"/> is a deterministic, and <img center src="http://latex.codecogs.com/gif.latex?
E(\epsilon_t) = 0.
" border="0"/>
Furthermore,
<img center src="http://latex.codecogs.com/gif.latex?
\mathcal{C}_t = g_t(S_t) + \eta_t,
" border="0"/>
where <img center src="http://latex.codecogs.com/gif.latex?
g_t
" border="0"/> is another deterministic function, and <img center src="http://latex.codecogs.com/gif.latex?
E(\eta_t) = 0.
" border="0"/>

<br>
The simplest (yet oftentimes tremendously helpful) state space model is the <i>linear Gaussian state state model. It is linear because we assume <img center src="http://latex.codecogs.com/gif.latex?
f_t
" border="0"/> is linear, it is Gaussian because we assume the noise is Gaussian. 
...
where <img center src="http://latex.codecogs.com/gif.latex?
\epsilon_t \sim Normal.
" border="0"/>

<h5>5. Bayesian Learning</h5>

Consider a model <img center src="http://latex.codecogs.com/gif.latex?
\mathcal{M}
" border="0"/>. With some a priori knowledge regarding the model (e.g. its probability distribution <img center src="http://latex.codecogs.com/gif.latex?
\mathbb{P}(\mathcal{M})
" border="0"/>; for example, <img center src="http://latex.codecogs.com/gif.latex?
\mathcal{M}
" border="0"/> could be of linear model with Gussian noise), and some parameters of interests (<img center src="http://latex.codecogs.com/gif.latex?
\theta_{\mathcal{M}}
" border="0"/> specific to the model and its distribution; e.g. <img center src="http://latex.codecogs.com/gif.latex?
\bold{A}
" border="0"/> and <img center src="http://latex.codecogs.com/gif.latex?
\bold{B}
" border="0"/> in the linear model above in section 4, or <img center src="http://latex.codecogs.com/gif.latex?
\mathbb{P}(\theta \vert \mathcal{M})
" border="0"/> in short).

<br>
We consider Bayesian learning in the sense that once we have the prior information, we could use data (<img center src="http://latex.codecogs.com/gif.latex?
\mathcal{D}
" border="0"/>) to update our posterior belief about the parameter of interest (<img center src="http://latex.codecogs.com/gif.latex?
\theta_{\mathcal{M}}
" border="0"/>):

<center>
<img center src="http://latex.codecogs.com/gif.latex?
\mathbb{P} (\theta \vert \mathcal{M}, \mathcal{D}) 
\propto
\mathbb{P} (\mathcal{D} \vert \theta, \mathcal{M}) \mathbb{P} ( \theta \vert \mathcal{M})  }
" border="0"/>
</center>

<br>
Now, we could use the previous dynamic connectivity matirces (e.g. <img center src="http://latex.codecogs.com/gif.latex?
\mathcal{D} := \{ \mathcal{C}_1, \cdots, \mathcal{C}_t \} 
" border="0"/>) to predict <img center src="http://latex.codecogs.com/gif.latex?
\mathcal{C}_{t+1}
" border="0"/> via the <i>predictive distribution</i>

<center>
<img center src="http://latex.codecogs.com/gif.latex?
\mathbb{P} (\mathcal{C}_{t+1} \vert \mathcal{D}) 
= 
\int \mathbb{P} (\mathcal{C}_{t+1} \vert \theta, \mathcal{M}, \mathcal{D}) \mathbb{P} (\theta \vert \mathcal{M}, \mathcal{D}) d \theta d \mathcal{M}
" border="0"/>
</center>


<h5>6. EM-algorithm and Statistical Inference</h5>

When there are no hidden states, the above parameters of interest <img center src="http://latex.codecogs.com/gif.latex?
\theta
" border="0"/> in Bayesian learning could be used in ML (maximum likelihood) estimation. The resulting estimates are ones that maximize the utility function (e.g a likelihood function).
...
<center>
<img center src="http://latex.codecogs.com/gif.latex?
\mathcal{L} (\theta) := \log \mathbb{P} (\mathcal{C} \vert \theta) 
= \log \sum_S \mathbb{P} (\mathcal{C}, S \vert \theta),
" border="0"/>
</center>
where <img center src="http://latex.codecogs.com/gif.latex?
S
" border="0"/> denotes the hidden state.

<br>
Then, for any random quantity <img center src="http://latex.codecogs.com/gif.latex?
Q(S)
" border="0"/>
<center>
<img center src="http://latex.codecogs.com/gif.latex?
...
<center>
 <img center src="http://latex.codecogs.com/gif.latex?
\mathcal{F}(Q, \theta):=
\sum_s Q(S) \log \mathbb{P} (S, \mathcal{C} \vert \theta) 
- \sum_S Q (S) \log Q(S)
" border="0"/>
</center>

<br> is the <i>free energy</i>.
...
<div id="top">
    <a id="logo" href="<?php echo SITE_URL?>" target="_blank">
        <img src="{{ site.baseurl }}/images/Coffee.jpg" alt="HTML5 Icon" style="width:250px;">
        </a>
</div>
</center>

<br> 
Finally, inference could be made based upon estimated paramters of interests <img center src="http://latex.codecogs.com/gif.latex?
\hat{\theta}
" border="0"/>. Certainly, if you are interested in obtaining a confidence interval for <img center src="http://latex.codecogs.com/gif.latex?
\hat{\theta}
" border="0"/>, you could bootstrap the data <img center src="http://latex.codecogs.com/gif.latex?
\{ \mathcal{C}_i \}_{ 1 \leq i \leq T}
" border="0"/> row-wise.
